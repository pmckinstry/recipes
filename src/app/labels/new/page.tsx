...
export default function NewLabelPage() {
  ...
  if (!session) {
    return (
      <MainContent>
        <div className="text-center text-red-600">You must be logged in to create labels.</div>
      </MainContent>
    );
  }
    ...
    <MainContent>
      <div className="max-w-lg mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <Link href="/labels" className="text-indigo-600 hover:text-indigo-800">← Back to Labels</Link>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-gray-900">Label Name</label>
            <input
              type="text"
              value={name}
